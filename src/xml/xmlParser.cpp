...
XMLNode *XMLNode::getChildNodePtr(LPCTSTR name, int *j)
{
    if (!d) return NULL;
    int i=0,n=d->nChild;
    if (j) i=*j;
...
XMLNode *XMLNode::getChildNodePtr(LPCTSTR name, int j)
{
    if (!d) return NULL;
    int i=0;
    while (j-->0) getChildNodePtr(name,&i);
    return getChildNodePtr(name,&i);
}
...
XMLNode      *XMLNode::getChildNodePtr (int i) { if (!d) return NULL;      if (i>=d->nChild    ) return NULL;      return &d->pChild[i];     }
